...
CUniFeederSocket::CUniFeederSocket(CDataSourceConnection &parent)
...
{  
	if (header_size==16)
	{
		ConvertToHeaderData(m_string);		
		if (get_data(m_string,_countof(m_string)-1,tick_size)==true )
		{
...
//+------------------------------------------------------------------+
bool CUniFeederSocket::ReadTick(char* str,MTTick &tick)
{				
   ...
   return(false);  
	...
	data_read_flag=0;
	header_size=16;
	if (m_st_tick->Exchange_code == CStaticClass::ExchangeCode)
	{
		CString  strsymbol=L"";
...
//+------------------------------------------------------------------+
bool CUniFeederSocket::DataGetString(char *buf,const int maxlen,int r_byte)
  {
   char *start=m_data+m_data_readed,*end=m_data+m_data_total;
   int   len=0;   
//--- checking parameters
...
void CUniFeederSocket::ConvertToHeaderData(char* st)
  {    
	   if(!st)
		  return;
	   st_Data_Details* m_st_Data_Details= reinterpret_cast<st_Data_Details*>(st);
	   tick_size=m_st_Data_Details->Un_com_Size;
	   header_size=tick_size;
